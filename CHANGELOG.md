...
### 新增
- 免費零門檻上手：按「連接 OpenRouter」一鍵登入授權就能對話，不必手動建立或貼上 API key；App 依角色扮演排行自動挑一支常駐免費模型（抓不到排行時退七日熱門），不選模型也能一直有免費對話。
- 設定頁顯示穩定免費前兩名（第一名自動、第二名可手動切換）與高上限的限時／實驗性免費模型，並附可關閉的新限免提示。
- 介面擴充到十國語言（繁體中文、简体中文、English、日本語、한국어、Español、Português (Brasil)、Deutsch、Français、Русский）：介面字典、首開範例桌內容、AI 輸出語言規範三處同步上齊。
- macOS 通用版（universal）打包：同一個 `.dmg` 同時支援 Intel 與 Apple Silicon Mac。
- `npm run verify`：統一驗證入口，一次跑前端建置＋Rust 測試＋i18n＋結構檢查。
- `npm run check:i18n`：語系字典體檢（佔位符一致性＋按鈕文字寬度），改文案或加語系後跑。
- 收回上一句：一次收掉紀錄最後一則、可連按往回收（只收當前這一幕）；訊息串底部的「復原」同樣可連按，把收掉的句子逐則倒回去。

### 變更
- 舊的 `smart_free` 選模設定在升級後自動遷移到新的動態穩定免費機制。
- 語系字典拆成每語言一檔（`src/i18n/`），範例桌內容抽成資料檔（`src-tauri/samples/`）：新增語系不必動程式邏輯。
- 切換語言時同步更新網頁語言標記，中日韓字形與斷行才會正確。

### 修正
- AI 回應被中途截斷時，保留已收到的半截內容，不再整段丟棄。
