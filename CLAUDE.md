...
- **回覆標記只用於 Claude 協作**：只有收到 Claude 轉交的訊息時，回覆才以 `DONE` 或 `QUESTION:` 收尾；使用者直接傳來的訊息不使用這兩個標記，照一般對話自然回覆。
...
- **新增檔案前先看 [docs/STRUCTURE.md](docs/STRUCTURE.md)**：新功能放哪、什麼時候升格資料夾、`shared/` 何時成立、改既有功能要不要順手搬，都寫在那裡。`npm run verify` 第一步會擋明顯違規。
