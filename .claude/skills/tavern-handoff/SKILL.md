...
收工本身不觸發 commit——工作項還沒做完就只更新上面四份檔案。

## commit 時機

以**工作項完成**為單位，不以收工為單位：一個可驗證的項目（實作＋自驗綠）做完就 commit 一次，訊息 `<任務 id>: 做了什麼（驗證結果）`，`.ai/` 的交接更新併進同一個 commit。做到一半停手不 commit。push 由使用者自行決定時機。
