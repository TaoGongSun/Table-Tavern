...
fn apply_merges_multi_source_person_deletes_exclusive_entries_and_sets_player_then_undo_restores() {
    ...
    let restored_bio = restored
        .iter()
        .find(|entry| entry.content == "亞瑟：劍術高超。")
        .unwrap();
    assert!(!restored_bio.is_person);
    assert!(restored
        .iter()
        .any(|entry| entry.content == "亞瑟：沉默寡言。"));
}
...
fn apply_rejects_second_player_card_and_writes_nothing() {
    ...
    assert_eq!(
        ...
        0
    ); // list_characters 排除玩家卡本人，新卡也沒寫入
    assert!(data::read_worldbook(root.path(), &world_id)
        .unwrap()
        .iter()
        .any(|entry| entry.uid == uid)); // 沒被刪
}
...
fn apply_unselected_person_gets_independent_person_entry_selected_persons_source_deleted() {
    ...
    let outcome = RefactorOutcome {
        mode: None,
        characters: vec![
            character("阿明", &[picked_uid]),
            character("小華", &[ignored_uid]),
        ],
        interface: None,
        mechanisms: Vec::new(),
        entries: Vec::new(),
    ...
    let ignored_entry = entries
        .iter()
        .find(|entry| entry.uid == ignored_uid)
        .unwrap();
...
fn apply_partial_group_selection_creates_person_entries_for_the_rest_and_keeps_shared_source() {
    ...
    let characters: Vec<RefactorCharacter> = names
        .iter()
        .map(|name| character(name, &[source_uid]))
        .collect();
    let outcome = RefactorOutcome {
        mode: None,
        characters,
...
fn apply_shared_uid_kept_when_not_all_owners_selected() {
    ...
    let outcome = RefactorOutcome {
        mode: None,
        characters: vec![
            character("霍玄", &[shared_uid]),
            character("長老", &[shared_uid]),
        ],
        interface: None,
        mechanisms: Vec::new(),
        entries: Vec::new(),
    ...
    assert!(data::read_worldbook(root.path(), &world_id)
        .unwrap()
        .iter()
        .any(|entry| entry.uid == shared_uid));
}
