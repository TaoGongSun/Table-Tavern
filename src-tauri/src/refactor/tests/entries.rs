...
fn apply_disables_whole_entry_drops_and_undo_restores() {
    ...
    let whole_uid = seed_entry(
        root.path(),
        &world_id,
        "格式",
        "<mainPage>卡片介面輸出協定</mainPage>",
    );
    ...
    let partial = worldbook
        .iter()
        .find(|entry| entry.uid == partial_uid)
        .unwrap();
...
fn apply_selected_rewritten_entries_creates_locked_mechanism_merges_rules_logs_and_deletes_shared_source(
    ...
    assert!(entries
        .iter()
        .any(|entry| entry.title == "新世界觀" && !entry.locked));
    assert!(entries
        .iter()
        .any(|entry| entry.title == "新戰鬥規則" && entry.locked));
...
fn legacy_outcome_without_entries_deserializes_and_applies() {
    ...
    assert_eq!(
        apply(root.path(), &world_id, &outcome, &selection)
            .unwrap()
            .summary
            .new_entries,
        0
