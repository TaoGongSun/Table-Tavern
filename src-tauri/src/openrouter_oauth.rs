...
pub(crate) fn save_openrouter_key(
    ...
    let config = persist_openrouter_key(&root, &api_key).map_err(|_| ERR_SAVE.to_owned())?;
...
pub(crate) async fn connect_openrouter(
    ...
    let config = persist_openrouter_key(&root, &key).map_err(|_| ERR_SAVE.to_owned())?;
    smart_free::warm(&app, &root, &config);
    Ok(config)
}
...
mod tests {
    ...
    fn bootstrap_uses_stable_free_mode_without_fake_api_tiers() {
        ...
        assert!(smart_free::is_active(&fresh));
        assert_eq!(
            fresh
                .preferences
                .get(smart_free::MODE_KEY)
    ...
    fn persisting_oauth_key_preserves_existing_config_and_adds_bootstrap() {
        ...
        for tier in ["best", "balanced", "fast"] {
            assert!(!saved.tier_models.contains_key(tier));
        }
        assert!(smart_free::is_active(&saved));
