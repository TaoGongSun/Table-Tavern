...
7. **保溫 ping**（已拍板可考慮，最後做）：距上輪近 4 分鐘且玩家還在（視窗聚焦／打字中）發迷你訊息刷新壽命；ping 後把垃圾訊息從 session 檔截掉（快取時鐘已被讀取刷新，截尾不影響）。**連發兩次 ping 仍無回應＝玩家離開，跳換幕提醒**（第二個換幕標準，見上）。細節實作時拍。
8. agy 量測（未拍板，擱置）。
...
2. **玩家離開太久**——保溫 ping 連發兩次仍無回應即建議換幕（快取只活 5 分鐘，長 transcript 每次回來都要全額重建，此時短紀錄才便宜）。隨包 7 保溫 ping 一起做。
