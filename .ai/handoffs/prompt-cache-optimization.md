...
- **包 6 額度分頁 UI**（2026-08-04，本次新完成；主線直寫，9 語系翻譯外包 codex luna）：
  ...
  - **下拉列出 app 現有的每一桌**（沒紀錄的照列、進去顯示 0），不是只列紀錄檔裡出現過的桌——後者會讓剛開的桌落不到選項上，select 退回顯示第一項，看起來像「選了總計卻是空的」（2026-08-04 使用者實機回報）。對不上現有桌的紀錄只剩兩種且都有看得懂的名字：`id` 空＝**開桌生成**（那時桌還沒建出來），`id` 有值但查無此桌＝**已刪掉的桌**。開發期沒帶桌別的舊行已於 2026-08-04 一次性改寫成「新的一桌」（使用者測試都在該桌），玩家不會遇到。
