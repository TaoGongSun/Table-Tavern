...
**2026-08-04 Opus 四輪真桌驗收全過＝包 1–5 架構驗收通過**（見 Verification）：2–4 輪 diag=ok、85.3／87.2／88.3%，讀到量與理論可中量每輪只差 2 個 token＝「只有本輪新內容沒中」的理想形狀，零 cache-skipped。Sonnet 兩場的爛數字全歸 CLI 官方 bug。官方 issue **不發**（2026-08-04 使用者拍板）。下一步＝包 6 額度分頁 UI。
...
- **包 7 保溫 ping**（2026-08-04，本次新完成；主線直寫）：
  ...
  - `App.tsx`：30 秒一次的計時器——距上次推進滿 3.5 分鐘、視窗在前景、沒有生成中才發；連三次（約 12 分鐘）沒等到玩家推進就停手並亮 `sceneAwayHint`。任何一次真正推進（角色接話／旁白／換幕）都會 `noteTurnDone()` 重置節奏並收掉提醒。視窗不在前景一律不發——人不在還持續扣錢是最糟的情況。
...
**2026-08-03 晚全部拍板完畢（案 C＋只做 claude，其他 CLI 日後分別實測）**。程式面包 1–5 完成且**架構已通過 Opus 四輪真桌驗收**；Sonnet 命中率受 CLI 官方 bug 壓制（2.1.220 仍在，取證見 Current state），app 端無事可做，等官方修。

**下一步＝包 6 額度分頁 UI**（規格見下方拆包第 6 項）。包 7 保溫 ping 已於 2026-08-04 先做完（使用者拍板調換順序，好讓額度分頁一併驗到保溫效果與 `ping` 分項花費）。
...
6. **包 6 額度分頁 UI**（2026-08-03 拍板，架構驗收已過，可開工）：設定頁「額度花費＋命中率」分頁，讀 JSONL log 畫花費＋命中率＋顏色燈號＋原因句。
...
2. **玩家離開太久**——保溫 ping 連發**三次**（約 12 分鐘，一次上廁所倒水的長度）仍沒等到玩家推進，就改跳「間隔太久、快取已清除」的提醒（`sceneAwayHint`，另一份文案）。已隨包 7 實作。
