...
**包 6 額度分頁完成並通過實機驗收**（2026-08-04）：設定頁「額度」分頁，收合只給長條圖＋快取比例＋花費，細項點開才出現。驗收九項通過、當場修掉六個顯示問題，grok 金額與 agy 那一列因無額度／CLI 計量未定延後（見 Verification）。**程式面包 1–7 至此全部完成。**
...
**通過（九項）**：分頁版面（收合只有下拉／數字行／長條圖）；數字與 `prompt-cache.jsonl` 手算逐欄吻合（21 輪 406,881／157,837／38.8%／$2.002，兩列模型加總＝總計列）；桌欄位有寫進新行、整份檔案零缺漏；使用中徽章跟著設定走；燈號句看過 warmup(first-turn／scene-changed)／expired／ok／single 四種；十語系畫面；**開桌生成認領**（一句話開桌生的新桌，三次生成呼叫 535／1,155／1,407 tokens 全數認到新桌）；**保溫 ping 實機成立**（讀 19,578／20,687＝94.6%、$0.017，自成一列且不進總計）；codex 那列無金額顯示「—」、總計標 `≥`。

**延後（兩項，2026-08-04 使用者拍板）**：grok 金額（暫無額度）、agy 那一列（該 CLI 的計量方式還沒確認）。程式面都有單元測試蓋著，等有額度／確認後補跑一輪即可。

**驗收當場修掉的六件事**（都已 commit）：下拉改列出 app 現有的每一桌（原本只列紀錄檔出現過的，剛開的桌會落空）；看所有桌總計時不出燈號句（那是某一桌「最近一輪」的事）；燈號句不再在收合處與細項各出現一次；俄文表頭撐寬視窗（縮短欄名＋表格自己橫向捲）；「使用中」徽章被折成兩行；「單發」文案改成先說是哪種呼叫（換幕摘要／開桌生成／不支援續聊的連線）。
...
- 包 7 保溫 ping：`cargo test` **214 passed**（含新 e2e：ping 後 session 檔逐字還原、下一輪續聊回覆編號仍為 2）、clippy 無新增警告、`npm run build`（含 tsc）＋`check:i18n`＋`npm test` 全綠。前端計時器 2026-08-04 實機驗過（保溫行真的出現、94.6% 命中）；**只剩「連三次沒回應＋紀錄超過 8000 字元才亮離開提醒」那條沒等滿 12 分鐘**。
...
實機驗收 2026-08-04 完成（九項過、六個顯示問題當場修掉）。**剩下的都不是本包必要工作**：
1. grok 金額、agy 那一列的顯示驗收（延後，等 grok 額度／agy 計量方式確認）。
2. 離開提醒那條（保溫連三次＋紀錄過長）沒等滿 12 分鐘，日後自然遇到再看。
3. OpenRouter（API 路）的實際扣款計量：官方可回報，本任務不接。
4. undo 截尾優化（`truncate_from` 已備好，收回上一句目前仍走重開線）。
5. claude CLI 的 Sonnet 快取 bug 等官方修（#29966），app 端無事可做。
