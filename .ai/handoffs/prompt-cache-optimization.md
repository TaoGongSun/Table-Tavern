...
- **包 6 額度分頁 UI**（2026-08-04，本次新完成；主線直寫，9 語系翻譯外包 codex luna）：
  ...
  - **選桌退場**：當前這桌在紀錄檔裡一筆都沒有（剛開的桌，或加桌欄位之前的舊資料全歸「未標桌」）時自動退回「所有桌總計」——下拉沒有那個選項時 select 會退回顯示第一項，看起來就像「選了總計卻是空的」（2026-08-04 使用者實機回報，已修）。
  - 細項收進 `<details>`「看細項」：燈號句（含時間戳）＋模型分行表（輪數／輸入／讀到快取／命中率／輸出／花費，使用中的模型帶徽章，保溫自成一列且淡化）＋總計列＋診斷次數 chip＋牌價註腳。
  - **燈號純規則**（`DIAG_KEYS`，對照 usage_log.rs 那張表）：ok／ping 綠、warmup／expired 黃、prefix-broken／cache-skipped／no-cache／drop-lane 紅、single 灰（單發不打分）。原因代碼另一張 `REASON_KEYS`（九個）。10 語系各 49 鍵。
