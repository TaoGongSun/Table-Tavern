...
- 日文「世界書」改用日本 AI 角色扮演圈慣用詞：定為「ロアブック」（跨工具通用度高於只在 SillyTavern 一家使用的「ワールド情報」），4 個鍵改寫，全檔無混用。
- 日文範例桌角色名「狐」維持不動。
- 範例桌地名各語系照母語習慣處理，不強求統一（德法創譯、日韓俄音譯、西葡保留 Mistmouth）。
- 角色性別：沒必要提就不提，語法強制時一律陽性。葡文狐狸從陰性改為「O Raposo」（含 world_md 與 opening 的相關句），其餘四個有語法性別的語系原本就是陽性，掃過無陰性殘留。
