...
**驗收延到全 app 功能定案後一次做**（2026-08-17 拍板）。現在不排、不動手。

原因＝驗收範圍已隨功能開發膨脹，原驗收單過期：

| | 驗收基準 2026-07-30（commit 9a17562） | 2026-08-17 現況 |
|---|---|---|
| 字典鍵數（zh-TW） | 247 | 474 |
| 受寬度檢查的按鈕 | 55 顆 | 102 顆 |
| 期間動過 `src/i18n/` 的 commit | — | 50 次 |

**最終項目數量無法預估**——功能仍在增加，每個新功能都會再往字典加鍵。等 app 功能全數完工才數得出要驗幾項、驗哪些畫面。

分層現況：
- **機械層持續有效**：缺鍵 TypeScript 編譯不過；`npm run check:i18n` 十語系佔位符一致、102 顆按鈕全在寬度上限內（2026-08-17 實跑綠）。
- **人眼層停在 07-30**：新增的 227 個鍵是各功能開發時順手補的，沒走過「Opus 逐語系審校」品質關卡；重構面板、狀態欄、贊助頁等新畫面沒人切到德俄日韓看過。

最終驗收時至少要涵蓋（07-30 原單三項＋後續新增畫面）：
1. 德文與俄文按鈕（最長語系）有無撐破或截斷；4 顆刻意放行的長鍵見上文 ACCEPTED_LONG。
2. 日韓字形與斷行（`document.documentElement.lang` 同步是否生效）。
3. 首開語言選單十個選項的排版。
4. 07-30 之後新增的功能畫面逐語系巡一次（清單屆時依實際功能重列）。

使用者拍板後才動的四件見下（已於 2026-07-30 全數拍板）。
