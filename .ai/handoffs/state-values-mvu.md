...
## Current state
2026-08-04：**八包全部完成**（包 1 標籤放寬、包 2 機制格式核心、包 3 `[initvar]` 匯入、包 4 增量解析＋本地權威＋統一協定、包 5 注入策略＋分支切割、包 6 全量型跳動標記、包 7 觸發表＋固定型 EJS 解析、包 8 未收編帳本），cargo test 317 綠、規範文件同步定稿。剩下的只有**真桌實跑**（開 app 打真模型跑幾輪，至今全是不呼叫模型的提示詞煙霧）與面板實機點過。
...
- 包 6 全量型跳動標記（規格與驗收主線，實作外派 general-purpose subagent `model: opus`）：
  - `TableState.jumps`（路徑 → `+40`／`-80`）跟著逐則快照走；只在 `!incremental` 的桌算，每輪重算。標記**只給面板看、不進提示詞**——全量桌注入逐字維持「不裁不濾不標」（標記進去等於教模型這欄可以大跳）。
  - 門檻兩條同時成立才算跳：絕對幅度 ≥30 且佔新舊值較大者四成。抽數字吃得下前後綴（`❤️ 60`、`第 3 天`、`500/500` 取現值），純文字欄不比。
  - `counter` 型不標；面板記號可點，點下去走新 command `mark_state_counter` 把該欄釘成 `counter` 規則（orc-cave 的時間跳躍是卡片明文功能，玩家點一次就永久靜音）。記一筆新的 `RecordKind::Jump` 進帳本。
- 包 8 未收編帳本（規格與驗收主線，實作外派 general-purpose subagent `model: opus`）：
  - `mechanism::read_ledger` 讀 `mechanism-log.jsonl`（檔案不在／壞行一律跳過）：`absorbed`／`skipped` 依 path 去重取最新，以標題對回現有世界書條目才進清單（對不上的例如事件旗標不列——沒有開關可切）；其餘四類只累計次數。
  - 面板入口＝世界書分頁的「機制帳本」折疊區塊，只在真的有東西可看時出現。每條附「照原文送模型」勾選（重用既有 `upsert_worldbook_entry` 反轉該條目停用），底下一行顯示拒收／夾限／格式錯誤／異常跳動次數。

## Verification
- 包 6／8：`cargo test` 317 passed（309→317，新增 8）；clippy 逐字持平（lib 8／lib test 9）；`cargo fmt --check` 本次碰的檔乾淨（既有 6 個不符 rustfmt 的無關檔維持原狀）；`npx tsc --noEmit`、`npm run build`、`npm run check:i18n` 綠。
- 包 6 真卡格式端到端測試（留在 repo）：donass 的 `<StatusData>` 平欄好感 20→70 標 `+50`、体力 -5 與中文數字欄「第一层」都不標；orc-cave 的 `<details>` 摺疊區塊「第 1 天→第 10 天」不標、「→第 60 天」標 `+50`（全形冒號與 `- ` 前綴都吃得下）。
- 包 8 勇者實卡帳本煙霧（跑完即刪，沒有呼叫任何模型）：匯入 100 條世界書→`import_mechanism`→`read_ledger` 列出 **30 條已接管**（21 條 EJS 觸發表＋`[initvar]`／三條 `[mvu_update]`／变量列表等）與 **9 條已跳過**，與包 7 煙霧記錄的 9 條認不出逐條吻合；全部 `sent=false`（都停用中，開關可打開）。
- 包 6／8 沒做：真桌實跑；面板兩塊 UI（跳動記號、帳本區塊）只驗到型別與編譯，沒實機點過。
...
## Remaining
只剩**真桌實跑**：開 app 匯入勇者卡（增量）與 donass／orc-cave（全量）各跑幾輪真模型，看回合尾字數、拒收自癒、觸發文本、跳動記號、帳本開關是否如預期。這會花使用者自己的額度，沒代做。
面板還沒實機點過的三處：分支指認下拉（包 5）、跳動記號（包 6）、機制帳本區塊（包 8）。
觸發表本身仍沒有檢視介面（帳本只列條目層級的接管／跳過）——真跑後若想看「這輪命中哪幾條」再補。
