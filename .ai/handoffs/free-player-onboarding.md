...
## Validation
- 2026-09-09 使用者在第一階段核心完成後曾完整跑 `npm run verify`，七步全綠：structure、cargo fmt、vitest、i18n、build、cargo check、cargo test。
- 首輪驗證曾抓到 rustfmt 差異與 `ulid 3.0.0` 沒有 `Ulid::new()`；已依本機結果修成 rustfmt 格式並將兩處 ULID 產生改為 `Ulid::generate()`，之後全綠。
- API format 收合小包完成後，使用者**再次完整跑 `npm run verify`，七步全部通過**。因此目前第一階段沒有待修的機械驗證項目。
- 尚未宣稱實際 OpenRouter OAuth／免費第一句已通過。

## Next action
1. 用全新 config 真實 OAuth 驗收：完全不碰 key／model id／高中低／付款，授權後直接送第一句並收到回覆。
2. 檢查落盤：全空 API tier → 三檔 `openrouter/free`；既有任一 API tier 自訂 → 三檔不被改；舊 `api_mode` 展開進階區後仍顯示原值。
