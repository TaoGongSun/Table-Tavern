...
- 玩家代號從中文「玩家」改成 `__PLAYER__`（人名撞不到）。GM 輪到玩家時吐代號／「玩家」／「Player」／玩家卡名字都對回代號，NPC 名字排前面故撞名算 NPC。前端比對改用常數 `PLAYER_SENTINEL`。
- GM 推進點到玩家時照樣寫一行「GM 請「布奇」發言」再停，不再默默停住讓人不知道為何卡著。
...
- 實機（使用者截圖佐證）：玩家卡「布奇」建立成功、側欄掛在 GM 卡正下方且皮革色與 GM 可分辨、NPC 對話認得玩家、玩家卡編輯畫面欄位與版面逐項調整到滿意。
- 實機（2026-07-28 最後一輪）：**GM 推進點到玩家會停下**，逐字稿留下「GM 請「布奇」發言」後把發言權交回玩家。這條原本是最大陷阱，已驗證。
