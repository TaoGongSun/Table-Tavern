...
## Remaining
**下次開工第一件事**：真 app 重載後點一次行動，看 GM 是否照卡片格式輸出、介面是否就地換成新畫面（會花使用者額度，由使用者決定何時跑）。若模型仍不照做，下一步是把格式規定條目的內容直接附在回合尾（現在只點名標題），代價是每輪多 5.4K 字元。

**待拍板（使用者第三輪實測後提出，皆延後處理）**
1. 模型改吐 XML 後，一般聊天畫面會顯示整段 XML（ST 那邊被 regex 換成 HTML 所以看不到）。要不要在聊天裡把這種訊息收成一行提示（例：「（卡片介面畫面）」）。介面本身不受影響。
2. **這類卡很燒額度**（使用者實測感受：一次回覆要好幾分鐘）——卡片要求模型每回合重吐整份狀態（小地圖、儲物袋、公會委託、角色表…）。可能的方向：本地權威已經做在 state-values-mvu（增量更新＋本地算術），能不能讓這類卡也只吐變動、由本地補完整份再餵給殼？需要先確認殼的渲染是否容得下「由我們回填的完整區塊」。屬 v2 優化，不擋 v1 收尾。

**原本的實機驗收清單**（真 app＝Tauri／macOS WKWebView；瀏覽器那關已過，剩下的是 WebKit 與真實匯入流程）：
