...
5. **介面裡按下去直接送出，畫面留在介面裡**（使用者拍板）：原本填完字就關掉面板讓玩家自己按送出，等於一回合把玩家踢出介面兩次，比 ST 差（ST 的介面嵌在對話裡，全程不必離開）。現在點下去就送，右上角顯示「GM 正在打字」，回覆到了就地重畫。卡片只按 `/trigger`（沒帶文字）＝直接要對象接話，不留玩家發言。
6. **不擋卡片自己觸發回合**（使用者質疑後查證改回）：我原本加了「玩家沒動作期間只准自動送一次」的煞車。查酒馆助手文件確認 ST 的作法是**每則訊息各自一個 iframe**、有「渲染深度」設定（從最新往回算渲染幾則，超出的移除），**文件裡沒有任何防迴圈或節流**。也就是說「載入就送」的卡在 ST 一樣會滾迴圈，作者不會這樣寫——我們沒有比 ST 多擋的理由。煞車與墊片裡的 isTrusted 偵測一併移除。
7. **純世界書桌沒有發言對象**（使用者第二輪實測）：介面按鈕送出的字有進逐字稿，但沒人接話。`enterTable` 原本是「挑第一個未封存角色」當對象，這種一個角色都沒有的桌對象是空字串 → `replyFromTarget` 什麼都不做，連輸入框都是鎖的。改成沒有角色時對象預設 GM。
8. **送出後介面不該關掉**（同上）：送出後最新一則變成玩家發言、算不出殼，`cardInterfaceShell` 變 null 導致覆蓋層直接關閉，看不到「GM 正在打字」。改成看 `cardShellReady`（現在有沒有殼可顯示，含雙緩衝前景那格），介面留著等回覆；切桌時把兩格緩衝一起清掉，免得上一桌的介面被當成這桌的。
9. **GM 沒照卡片格式輸出**（使用者第三輪實測，最關鍵的一個）：介面永遠停在開場畫面。根因是兩邊指令打架——卡片的『回复规则』條目（`constant`，5,432 字）寫「每次回复必须且只能输出一个 XML 数据块，禁止其他内容」，而我們在**回合尾**（最後說話、最強位置）要求「插入一段旁白＋```state 圍欄」。模型聽我們的 → 吐散文 → 卡片 regex 對不上 → 介面不動。
   - 修法：`gm_narrate` 先看這桌有沒有可渲染的卡片介面（`read_card_interfaces`），有就把導演指示換成 `transport::card_format_instruction`——要求完全照卡片規定輸出，不要旁白也不要 state 圍欄、不要「下一位」點名行；`closing` 一併換掉。沒有卡片介面的桌**逐字不變**。
   - 點名哪一條規則是自動找的：`import::card_format_entry` 從顯示腳本的 `find_regex` 抽出字面標籤（`<GoldenRPG_UI>`、`<CurrentView>`…），回世界書找內容含這些標籤、未停用的條目，取命中標籤最多者。
   - **副作用待處理**：模型改吐 XML 後，一般聊天畫面會顯示整段 XML（ST 那邊是被 regex 換成 HTML 所以看不到）。介面照樣正確（面板吃的是 `raw`）。要不要在聊天裡把這種訊息收成一行提示待拍板。
10. **雙緩衝**（同上脈絡）：新殼先在背面那格 iframe 載好才翻面，中途舊畫面一直在、不會閃白。兩格的 `key` 固定成格號——換 key 會重新掛載等於白載一次。分頁與捲動位置仍會重置（新文件必然如此，ST 也一樣）。
...
## Current state
2026-08-04 v1 三包＋使用者三輪實測的修正全部完成、自驗全綠（cargo test 285／vitest 17／tsc／check:i18n／build）。真卡實機已跑到「匯入→開介面→點行動→送出→GM 回覆」整條路；最後一項（GM 改照卡片格式輸出後介面會不會就地換新畫面）改完尚未實測，**下次開工第一件事就是這個**。
...
- **包 2 前端渲染**（規格與驗收主線，實作外包兩隻 general-purpose subagent `model: opus`，一隻純函式一隻畫面）：
  - 新檔 `src/features/card-interface/interface-card.ts`：`parseStRegex`（`/樣式/旗標` 與裸樣式兩種寫法，丟掉 JS 沒有的旗標，壞樣式回 null）、`applyScripts`（**一律用 replacer 函式**手動掃 `{{match}}`／`$1`–`$9` 單輪代換——殼有 42KB 且內含 jQuery `$(`、CSS 與 `$&`，交給 JS 原生 `$` 語意會整份壞掉；單支腳本壞掉跳過續走）、`extractShell`（先找 ```` ```html ```` 圍欄、退而找裸 `<!DOCTYPE html>`）、`buildShellDocument`（插入橋接墊片＋把殼裡 `window.parent`／`window.top` 字面改指墊片）。
...
## Remaining
✅ **v1 實機驗收全數通過**（2026-08-04 使用者實測）：匯入 → 開介面 → 點行動 → 直接送出 → GM 照卡片格式回覆 → **介面就地換成新畫面**。

**已拍板不做**：聊天區把 XML 收成一行提示。玩家正常不會關掉介面，關掉後看到什麼都無所謂；而且顯示層收合**省不到任何額度**。

**v2 首要項目：省額度（使用者實測感受「一次回覆要好幾分鐘」）**
- 真正的成本在**歷史**：每輪的整包 XML 原封留在 transcript，下一輪連同前面每一輪一起送模型。玩十輪＝重送十份完整狀態表。（送模型的是 `event.text`，而我們的狀態剝殼認不得 `<GoldenRPG_UI>` 這種卡片自訂標籤，所以整包留著。）
- ST 那邊部分卡自帶 `promptOnly` 的「不發送前文狀態欄」規則在解同一件事——**尋道太虛卡有，西幻卡沒有**。
- **坑**：劇情正文就在資料塊裡（`<CurrentView><Story>`），整塊砍掉等於把故事從模型記憶抹掉。要做的是「留正文、砍掉重複的地圖矩陣／委託清單／角色表」，屬結構性裁剪，需要一包好好做。
- 另一條路：沿用 state-values-mvu 的本地權威（增量更新＋本地算術），讓模型只吐變動、由本地補回完整區塊再餵給殼——要先確認殼吃不吃「我們回填的區塊」。

**原本的實機驗收清單（已全數通過，留作回歸檢查）**（真 app＝Tauri／macOS WKWebView；瀏覽器那關已過，剩下的是 WebKit 與真實匯入流程）：
