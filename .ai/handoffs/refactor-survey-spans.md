...
## 包序（每包 sonnet 代理實作＋主線複驗，完成即 commit）
1. **包1** span 切分＋⟦sN⟧ 標記＋預掃＋新 SURVEY_BODY＋六區塊解析器＋structs＋測試（PLAN 區塊與 RefactorPlanEntry 刪除）。驗收：cargo 綠、system 相等測試不動、壞行略過測試。
2. **包2** refactor_assemble_local＋四稽核＋RefactorNewEntry.meta＋apply() 元資料保留。驗收：byte 相等＋稽核測試。
3. **包3** absorb_messages/parse＋group_messages/parse＋`{{span:…}}` 替換＋大組保險＋tauri commands（refactor_rewrite_entry 舊中段刪除）。驗收：解析測試＋system 相等測試擴充。
4. **包4** 前端：chain 廢除全並行（refactor-run.ts 加 warmed 直進 pool）、新流程接線、knownFields 改小抄 FIELDS、dedup、TS 型別同步。驗收：vitest＋build。
