...
## Current state
2026-08-11 包 1–3 實作完成，代理自驗＋主線複驗全綠（cargo **442**／vitest **94**／npm build 0／check:i18n 0），三個 commit：包 1 `cce182d`、包 2 `60c58f2`、包 3 見 git log。同日實機開跑 orc-cave：檔位與快取生效（P1／P3 綠），但總時長仍 ~24 分（P2 紅）且盤點分類不合格（P7 紅）——提速與品質改由 [refactor-survey-spans](../tasks/refactor-survey-spans.md) 接手（設計與證據在該任務檔），本案僅剩取消類驗收（P4–P6、P8）等新案完成後合併驗。
...
- [x] P7 品質 **✗**（基線拍板 b＝絕對標準）：純設定條目（豺狼人／深藍狼／巨魔等）被整篇重寫；逐日機制「巴古克與古茲卡入侵劇情線」未接管。分類與產出策略缺陷，修法見 [refactor-survey-spans](../tasks/refactor-survey-spans.md)。
...
## Next action
先做 [refactor-survey-spans](../tasks/refactor-survey-spans.md)（新對話）；完成後回本案補驗 P4–P6／P8，過即結案（狀態 completed、TASKS.md 行搬 DONE.md、本檔已驗收段搬 archive/）。
