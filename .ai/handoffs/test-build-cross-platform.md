...
Updated: 2026-07-24T14:45:00+08:00
Status: in_progress
...
## Current state
產線可用，每次下令即可重打。最新一輪 0.2.0（2026-07-28 16:36，HEAD 9d71876）：Mac `src-tauri/target/release/bundle/dmg/Table Tavern_0.2.0_aarch64.dmg`（4.7MB，codesign `adhoc,runtime`，乾淨路徑掛載後 `--verify --deep --strict` 通過）＋Windows [run 30342814136](https://github.com/TaoGongSun/Table-Tavern/actions/runs/30342814136) success、artifact `table-tavern-windows-unsigned` 含 `Table Tavern_0.2.0_x64-setup.exe`（NSIS 3.3MB）與 `Table Tavern_0.2.0_x64_en-US.msi`（4.7MB）。剩使用者實機驗收：Mac DMG 拷去 MacBook Air 測 Gatekeeper、Windows artifact 在真 Windows 機安裝。
