...
## Current state
三件全數實作完成，cargo 96 綠＋npm build 綠，等使用者實機實測拖曳手感後結案。瀏覽器預覽驗證不適用（App 開場即呼叫 Tauri `invoke`，純 vite 跑不起來），僅靠單元測試與型別檢查。
...
- **CSS**：`.tcard` 加 `user-select: none`（App.css:506）、`.worldbook-row` 加 `cursor: grab`＋`user-select: none`（App.css:1420-1421）、拖曳中共用 `.row-dragging`（浮起：scale 1.02＋加深陰影＋grabbing 游標，App.css:510）。
...
- 使用者 `npm run tauri dev` 實測：角色卡跨多格拖曳、拖完不會誤選發言者、GM 卡不動如山、世界書拖曳與列不反白。
