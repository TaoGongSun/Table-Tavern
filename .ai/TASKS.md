...
## In progress
- [state-values-mvu](tasks/state-values-mvu.md) — 狀態欄二期：機制格式（IR）＋本地權威數值＋觸發表 — 下一步：包 1–4 完成（2026-08-04，cargo test 262 綠、勇者實卡煙霧驗過，數值已由本機記帳）；剩包 5–8，下次挑包 5（注入策略＋分支切割）
...
## Todo
- [interface-card-panel](tasks/interface-card-panel.md) — 介面卡渲染面板：ST 介面卡原樣顯示（殼匯入＋沙盒面板） — 下一步：2026-08-04 拆西幻卡立案；排程待拍板（建議 state-values-mvu 包 4／5 後），「transcript 加存原文」包 4 已帶到（2026-08-04）
