...
- [prompt-cache-optimization](tasks/prompt-cache-optimization.md) — 提示詞快取優化：resume 續聊架構（claude lane） — 下一步：包 1＋包 2 完成（2026-08-03，cargo test 201 綠，claude 模式已走續聊線）；接著包 3 凍結快照補丁（先拍板角色檔位混用議題）→ 包 4 log → 包 5 GM 合併，整包完成後真桌驗收 hit_rate ≥66%
