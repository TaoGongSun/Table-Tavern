...
- [release-1-mac-signing](tasks/release-1-mac-signing.md) — 發佈 1：Mac 正式簽章＋公證（Developer ID＋notarytool） — 下一步：先做不用等的：repo 加 AGPL-3.0 LICENSE（§16 拍板）。簽章部分等使用者加入 Apple Developer Program（99 美元/年）後，設 Developer ID 憑證＋notarytool 公證流程，本機驗證通過後把憑證交接給 release-2 的 CI secrets
