...
## In progress
- [interface-card-panel](tasks/interface-card-panel.md) — 介面卡渲染面板：ST 介面卡原樣顯示（殼匯入＋沙盒面板） — 下一步：2026-08-04 v1 三包＋三輪實測修正完成（cargo test 285／vitest 17／tsc／i18n／build 全綠），真卡已跑到「匯入→開介面→點行動→送出→GM 回覆」；下次第一件事＝實測 GM 改照卡片格式輸出後介面會不會就地換新畫面，另有兩項待拍板（聊天顯示 XML、這類卡很燒額度的優化）
