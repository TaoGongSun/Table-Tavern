...
- [prompt-cache-optimization](tasks/prompt-cache-optimization.md) — 提示詞快取優化：穩定前綴重構＋命中率量測＋Claude 顯式斷點 — 下一步：A＋B＋C 三塊全部完成（cargo test 168 全綠，2026-08-03）；等實機驗收（終端機啟動看 stderr 的 [prompt-cache] 命中率、隱式與 Claude 系都應 >0＋比對搬尾端後敘事品質），過後只剩命中率正式 UI 待拍板
