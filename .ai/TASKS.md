...
## In progress
- [interface-card-panel](tasks/interface-card-panel.md) — 介面卡渲染面板：ST 介面卡原樣顯示（殼匯入＋沙盒面板） — 下一步：2026-08-04 v1 三包全完成，cargo test 277／vitest 14／tsc／check:i18n／build 全綠；真瀏覽器互動實測過（西幻卡介面完整渲染、推薦行動與開場「踏入命运」兩條路的按鈕文字都送回輸入框、連點不黏字、console 無錯）；使用者第一輪實測回報「按鈕沒出現」已修（根因：世界書卡判定條件錯，匯成角色卡會丟掉整包設定條目）；並補上匯入完自動打開介面一次；等重開一桌重匯實卡驗收
