...
- [prompt-cache-optimization](tasks/prompt-cache-optimization.md) — 提示詞快取優化：穩定前綴重構＋命中率量測＋Claude 顯式斷點 — 下一步：A＋C 完成（cargo test 167 全綠，2026-08-03）；等實機驗收（終端機啟動看 stderr 的 [prompt-cache] 命中率＋比對搬尾端後敘事品質），B 等拍板優先度
