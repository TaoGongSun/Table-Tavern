...
- [cli-install-windows](tasks/cli-install-windows.md) — CLI 一鍵安裝 Windows 支援（PowerShell 分支） — 下一步：四家登入已改可見終端機視窗＋刪除 headless 撈 URL 機制（cargo 73 綠），等使用者下令跑 CI 驗證＋重打包後給朋友複測
