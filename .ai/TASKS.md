...
- [api-shared-lane](tasks/api-shared-lane.md) — API 路徑改走 chars 共線：讓換角色不再打散前綴快取 — 下一步：實機 runtime 驗收（要使用者在電腦前）：真模型角色辨識三項（錯認前言者／串角／私設洩漏，只需在 API 路徑驗）＋四路成對測試（同角色／換角色 × 冷／暖），記絕對 cached tokens，codex 要先扣掉固定的 9,984。
