...
## In progress
- [interface-card-panel](tasks/interface-card-panel.md) — 介面卡渲染面板：ST 介面卡原樣顯示（殼匯入＋沙盒面板） — 下一步：2026-08-04 開工分三包，包 1 Rust 讀取端完成（cargo test 277 綠）；面板拍板為全螢幕覆蓋層、只渲染最新一則；下次做包 2 前端渲染（`src/interface-card.ts` 三個純函式起）
