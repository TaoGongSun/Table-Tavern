...
Updated: 2026-08-04T02:25:00+08:00
...
- 換幕提醒改標準（已做，10 語系綠）：字數門檻 8000→30000 並換掉文案理由；第二標準「保溫 ping 連三次沒等到玩家＝離開太久、快取已清」另寫一份文案。
- **包 7 保溫 ping 完成**（拍板提前做，好讓額度分頁一併驗到保溫效果）：距上次推進 3.5 分鐘且視窗在前景就替所有活著的線讀一次快取續命，問答隨即從 session 檔截掉不留痕跡；連三次（約 12 分鐘）沒等到玩家就停手改提示換幕。cargo test 214 綠。
- **下一步：包 6 額度分頁 UI**（設定頁讀 JSONL 畫花費＋命中率＋燈號＋原因句，含 `ping` 分項花費）。
