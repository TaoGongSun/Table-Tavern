...
Updated: 2026-07-31T02:00:00+08:00
...
- 2026-07-31 兩輪修正待進包：①「檔案被佔用」安裝失敗白話提示（回報者 Grok 案一，防毒／殘留程序鎖檔）；②系統代理自動下傳＋連線失敗提示（案二「空白登入視窗」，梯子只開系統代理時 CLI 吃不到代理；回報者手動設 HTTPS_PROXY 後登入成功實證）。CI verify → 重打包 → 回報者只開系統代理走安裝→登入→聊天全流程（聊天必驗）；細節見 ../handoffs/cli-install-windows.md
